...
export function ButtonList({
  ...
  const calculateCost = (guard: GuardButtonList, quantity: number) => {
    if (guard.label === "SOL") {
      // Assuming 0.18 SOL per NFT, adjust this value as needed
      return (0.18 * quantity).toFixed(2) + " SOL";
    } else if (guard.label === "AURA") {
      // Assuming 100K AURA per NFT, adjust this value as needed
      return (100000 * quantity).toLocaleString() + " $AURA";
    }
    return guard.mintText; // Default to original mintText if not SOL or AURA
  };
